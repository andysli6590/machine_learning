...
def extract_train_labels(filename):
    ...
    V = [Y[i][0] for i in range(0, len(Y))]
    return V
...
################################-----Learning algorithm-----###########################################
def liblinear_train():
	p1 = subprocess.call([r"./train","./train.data", "./models"], stdout=subprocess.PIPE)

def liblinear_test():
	p2 = subprocess.call([r"./predict", "./test.data", "./models", "./predictions"], stdout=subprocess.PIPE)
...
if __name__ == '__main__':
	...
	liblinear_train()										# learn a model with train data and create model file

	liblinear_test()										# read test data, model and output predictions
